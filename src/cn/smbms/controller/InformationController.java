...
import cn.smbms.service.dataDatasictionary.DataDictionaryService;
import cn.smbms.service.information.InformationService;
import cn.smbms.service.uploadtemp.UploadTempService;
import cn.smbms.tools.Constants;
import cn.smbms.tools.JsonDateValueProcessor;
		...
		if(session.getAttribute(Constants.SESSION_BASE_MODEL) == null){
			...
			try {
				...
				if(null != information.getTitle() && !information.getTitle().equals("")){
					logger.debug("======= addInformation HtmlEncode.htmlEncode(information.getTitle())================" + information.getTitle());
					information.setTitle(information.getTitle());
				}
	...
	public ModelAndView modifyInformation(@ModelAttribute("modifyInformation") Information information,HttpSession session){
		if(session.getAttribute(Constants.SESSION_BASE_MODEL) == null){
			...
			try {
				...
				information.setUploadTime(information.getPublishTime());
				if(null != information.getTitle() && !information.getTitle().equals("")){
					logger.debug("======= modifyInformation HtmlEncode.htmlEncode(information.getTitle())================" +information.getTitle());
					information.setTitle(information.getTitle());
				}
